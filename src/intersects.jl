...
function Base.intersect(l::Line, a::AxisRect)
    s = sides(a)
    it = l ∩ s[1]
    il = l ∩ s[3]
    ib = l ∩ s[2]
    ir = l ∩ s[4]
    if !isnothing(it) && !isnothing(ib)
        return Segment(it, ib)
    end
    if !isnothing(il) && !isnothing(ir)
        return Segment(il, ir)
    end
    if !isnothing(il) && !isnothing(ib)
        return Segment(il, ib)
    end
    if !isnothing(ib) && !isnothing(ir)
        return Segment(ib, ir)
    end
    if !isnothing(ir) && !isnothing(it)
        return Segment(ir, it)
    end
    if !isnothing(it) && !isnothing(il)
        return Segment(it, il)
    end
    return nothing
end
